      ...
      <div class="content-wrapper">
        ...
        <section class="content">
          ...
          <?php
            ...
            $refilado='';
            $refilado = "SELECT * FROM refilado_requisitos WHERE num_orden = '$pedido'";

            $registrosRef = sqlsrv_query($connSCPBD,$refilado);
            if( $registrosRef === false ) {
            die( print_r( sqlsrv_errors(), true));
            }
              $datos = sqlsrv_fetch_array($registrosRef);
              $refilado = explode(', ', $datos['refilado']);

               $registrosLam_ = sqlsrv_query($connPlas,$leerRefi);
        ...
        <br>
           ...
           <!--- Detalles Orden de Produccion Laminacion -->
                      <br>
                              ...
                              <div class="table-responsive"> 
                                ...
                                <th>ALTURA DE LA IMPRESION<input type="checkbox" class="chkplas" class="minimal" value="chk_impresion" <?php if(in_array('chk_impresion',$refilado)){echo 'checked="checked"';}?> name="chkrefilado[]" ></th>
                                    ...
                                    <?php 
                                        //echo $datos['fechare'];                                        
                                      $fechabien = date_format($datos['fechare'], 'Y-m-d');

                                     ?>
                                    
                      <tr>
                            <td><input type="date" min=2014-01-01 class="form-control" value="<?php echo $fechabien ?>" name="fechrefi">

                              </td>
                            <td><input type="text" class="form-control" value="<?php echo $datos['kilos'] ?>"  name="krefi"></td>
            ...
            <div class="row">
                    ...
                    <div class="col-md-4">
                        <h4 align="center">CALIBRE (mp)<input type="checkbox" class="chkplas" class="minimal" value="chk_calibre" <?php if(in_array('chk_calibre',$refilado)){echo 'checked="checked"';}?>name="chkrefilado[]" ></h4>
                            <div class="table-responsive"> 
                                <table class="table table-bordered"> 
                                    <thead> 
                                        <tr> 
                ...
                <div class="col-md-12">
                  <div class="table-responsive"> 
                       ...
                       <td><?php echo $imgMontaje ?></td>
                          ...
                          <td>
                            <?php $obsCal = "SELECT * FROM dbo.REFILADO rf inner join  dbo.OBSCALIDAD cal on rf.NIT = cal.NIT where rf.NIT = '$fila->NIT' and proceso = 'REFILADO' and rf.ORDENNRO = '$pedido' and ((cal.codigo='$fila->CODIGO)') or (cal.codigo=''))";
            ...
            <div class="row">
               <div class="col-xs-4">
                    <label>Tiempo de Curado</label>
                     <input  type="text" class="form-control"  value="<?php echo $datos['tcurado'] ?>" name="tcurado" > 
                      <label>Gramos/m2</label>
                     <input  type="text" class="form-control"  value="<?php echo $datos['gramosm'] ?>" name="gramosm" >

                    <b>Operario responsable</b>
                      <i id="limpiar" style="cursor: pointer;" class="fa fa-times"></i><br><br>
                      <input id="operarios" type="text" class="form-control" placeholder="Nombre Operario" value="<?php echo $datos['operario']?>" name="operario" required> 
               </div>
               <div class="col-xs-9">
                 <br>
                  <div class="col-md-2">
