<?php 
  ...
  $registrosLamFilas = sqlsrv_query($connPlas, $comprobacionLam, array(), array( "Scrollable" => 'static' ));
  ...
  $validarLam = sqlsrv_query($connSCPBD, $contadorLam);
...
if ($fila->Total == 0){
  if ($contLamFilas != 0){
      ...
      echo    "<form method='post' action='http://".$_SERVER['SERVER_NAME']."/apps/laminacion/control_requisitos/nuevo_control_requisito.php'>
          ...
          <div class='col-md-8'>";
      echo "<b>Información:</b> este pedido se encuentra disponible para ser gestionado, para realizar premontaje presione el botón aceptar.";
      echo    "<form method='post' action='http://".$_SERVER['SERVER_NAME']."/db/laminacion/control_requisitos/nuevo_control_requisito.php'>
                  <input name='pedido' type='hidden' value='".$pedido."'>
                  <input type='submit' class='btn btn-info' name='submit' value='Aceptar'>
              </form></br>";*/
  }
